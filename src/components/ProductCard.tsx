          ...
          <div className="absolute inset-0 bg-foreground/0 group-hover:bg-foreground/10 transition-colors duration-300 flex items-end justify-center p-4 opacity-0 group-hover:opacity-100">
            <div className="w-full flex flex-col gap-2">
              <div className="flex gap-1 justify-center">
                {product.sizes.map((s) => (
                  <button key={s} onClick={() => setSelectedSize(s)} className={`w-9 h-9 text-xs font-display font-bold transition-colors duration-200 ${selectedSize === s ? "bg-foreground text-background" : "bg-background text-foreground hover:bg-accent"}`}>
                    {s}
                  </button>
                ))}
              </div>
